...
The dependent variable is tored in the matrix **Y_mig** of size $21 \times 51$.
...
$$(I_{N\times N}-\rho_dW_dy-\rho_oW_oy-\rho_wW_wy)Y = \iota_N\alpha + vec(I_n)\alpha_i + X_d\beta_d+X_o\beta_o+X_i\beta_i + XL_d\delta_d + XL_o\delta_o + XL_i\delta_i + \gamma G+\epsilon $$
...
with $Z=(1_{N}, vec(I_n), X_d, X_o, X_i, XL_d, XL_o, XL_i, G)$, $\delta = (\alpha, \beta_d, \beta_o, \beta_i, \delta_d, \delta_o,  \delta_i, \gamma)$. We generate a set of flows $Y$ with $\alpha = 1$, $\alpha_i = 0.5$, $\beta_d = 1$, $\beta_o = 0.5$, $\beta_i=2$, $\delta_d = 0.25$, $\delta_o = 0.1$, $\delta_i = 0.5$, $\gamma = -2.0$, $\rho_d = 0.4$, $\rho_o = 0.4$, and $\rho_w = -0.16$.
...
* **intra_x**, a boolean which includes $X_i$. 
...
### Comparison of computationnal time  ###
...
* *gravity_model()* is faster than *lm()* function. 
...
* **x_d**, a **data.frame** or a matrix with explanatory variables observed on the $n_d$ geographical sites.
* **x_o**, a **data.frame** or a matrix with explanatory variables observed on the $n_o$ geographical sites.
* **ind_d**, the indices of the variables in **x_d** which will be used at the destination,
* **ind_o**, the indices of the variables in **x_o** of the variables used at the origin,
* **lagged**, a boolean which includes if **T** the lagged explanatory variables,
* **centered**, a boolean which centered the explanatory variables and uses the simplified formulas, 
...
```{r, echo = F}
time_au_lagsarlm <- system.time(lagsarlm(y_2 ~ x_d + lagged_x_d + 
                                           x_o + lagged_x_o + g, 
                      data = flows_au, mat2listw(W_au_d)))
time_ge_lagsarlm <- system.time(lagsarlm(y_2 ~ x_d + lagged_x_d + 
                                           x_o + lagged_x_o + g, 
                      data = flows_ge, mat2listw(W_ge_d)))
time_usa_lagsarlm <- system.time(lagsarlm(y_2 ~ x_d + lagged_x_d + 
                                            x_o + lagged_x_o + g, 
...
```{r, eval = F}
result_s2sls<- stsls(y_2 ~ x_d + lagged_x_d + x_o + lagged_x_o + g, 
                           data = flows_au, 
                           mat2listw(W_au_d))
...
```{r}
(sar_simu_2_sls <- s2sls_flow(x_d = flows_au[, c("x_d", "lagged_x_d")],
...
time_ge_stsls <- system.time(s2sls_flow(
  x_d = flows_ge[, c("x_d", "lagged_x_d")],
  x_o = flows_ge[, c("x_o", "lagged_x_o")],
  y = flows_ge$y_2, 
  g = flows_ge$g, 
  instru_x_d = c(F, T),
  instru_x_o = c(F, F),
...
time_usa_stsls <- system.time(s2sls_flow(
  x_d = flows_usa[, c("x_d", "lagged_x_d")],
  x_o = flows_usa[, c("x_o", "lagged_x_o")],
  y = flows_usa$y_2, 
  g = flows_usa$g, 
  instru_x_d = c(F, T),
  instru_x_o = c(F, F),
...
system.time(sar_simu_9_method2 <- sar_flow_2(x = flows_au[, 
                                        c("x_d", "lagged_x_d",                                                              "x_o", "lagged_x_o")], 
                                     y = flows_au$y_9, 
                                     g = flows_au$g, 
