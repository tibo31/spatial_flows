...
##### Storing the explanatory variables into origin-destination format 
...
We prepare a grid of origins and a grid of destinations. The explanatory variables at origin and at destination are not necessarly the same. 
...
### Comparison of computationnal times according to size ###

```{r, echo = F}
time_lm_full_au <- system.time(gravity_model(x = as.matrix(spdf_au@data$x), 
                                             Y = Y_au_0, G = G_au))
time_lm_full_ge <- system.time(gravity_model(x = as.matrix(spdf_ge@data$x), 
                                             Y = Y_ge_0, G = G_ge))
time_lm_full_usa <- system.time(gravity_model(x = as.matrix(spdf_usa@data$x), 
                                             Y = Y_usa_0, G = G_usa))
time_lm_vect_au <- system.time(lm(y_0 ~ x_d + x_o + g, data = flows_au))
time_lm_vect_ge <- system.time(lm(y_0 ~ x_d + x_o + g, data = flows_ge))
time_lm_vect_usa <- system.time(lm(y_0 ~ x_d + x_o + g, data = flows_usa))
(time_gravity <- data.frame(matrix = c(time_lm_full_au[3], time_lm_full_ge[3], 
                                      time_lm_full_usa[3]),
                           vector = c(time_lm_vect_au[3], time_lm_vect_ge[3],
                                      time_lm_vect_usa[3]),
