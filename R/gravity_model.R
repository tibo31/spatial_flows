gravity_model <- function(x, Y, G, ind_d = NULL, ind_o = NULL) {
  ...
  zprime_z <- rbind(c(N, zero_k_d, zero_k_o, iota_n %*% G %*% iota_n),
                    cbind(zero_k_d, 
                          n * crossprod(x_matrix_centered[, ind_d]), 
                          zero_k_d_o, 
                          crossprod(x_matrix_centered[, ind_d], G %*% iota_n)),
                    cbind(zero_k_o, 
                          zero_k_o_d, 
                          n * crossprod(x_matrix_centered[, ind_o]), 
                          crossprod(x_matrix_centered[, ind_o], G %*% iota_n)),
                    cbind(iota_n %*% G %*% iota_n, 
                          iota_n %*% crossprod(G, x_matrix_centered[, ind_d]), 
                          iota_n %*% crossprod(G, x_matrix_centered[, ind_o]), 
                          sum(diag(crossprod(G))))
  )
  
  # We code the formula 83.4 in Thomas and LeSage (2014) :
  z_prime_y <- rbind(iota_n %*% Y %*% iota_n,
                     crossprod(x_matrix_centered[, ind_d], Y %*% iota_n),
                     crossprod(x_matrix_centered[, ind_o], t(Y) %*% iota_n),
                     sum(diag(G %*% Y)))
